...
function coefficients_unsaturated(N::SubquoModule{T}, p::FaceQ) where {T <: MonoidAlgebraElem}
  kQ = base_ring(N)
  ...
  for b in Bp
    ...
    # submodule generated by the relevant generators 
    ...
    for r in rel_rels
      ...
      #can _r be written using generators in rel_gens
    ...
    #get kernel of coefficient matrix of relevant relations
    if !is_empty(C_bF)
    ...
    #get kernel of coefficient vector of b
    ...
    for c_K in rows_K
      ...
      if rank(B_K)>l #check if c_K is not in the kernel of b
  ...
  return _Bp, matrix(kQ,map(kQ, hcat(lambda...))) #something goes wrong here
end
...
function coefficients(N::SubquoModule{T}, p::FaceQ) where {T <: MonoidAlgebraElem}
  ...
  for b in Bp
    ...
    for g_N in filter(!is_zero, gens(N))
      ...
      if dim(intersect(b_p + p.poly + (-1)*p.poly, g_p + cone(kQ))) >= 0 #maybe this goes wrong??
        push!(G_b, g_N)
      end
    end
